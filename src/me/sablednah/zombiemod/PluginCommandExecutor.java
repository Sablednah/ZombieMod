			...
			if (args.length > 0 && args[0].toLowerCase().equals("stats")) {
				...
				for (World w : plugin.getServer().getWorlds()) {
					...
					for (Zombie e : zombies) {
						...
						if (zomb!=null) {
							ZombieMod.logger.info("[" + ZombieMod.myName + "] " + zomb.species + " | " + zomb.commonName + "  [" + Math.floor(e.getLocation().getX()) + " X - " + Math.floor(e.getLocation().getY())+ " Y - " + Math.floor(e.getLocation().getZ())+ " Z]" );
						}
					}
				}
			    ...
			    while (it.hasNext()) {
			        ...
			        System.out.println(entry.getKey() + " = " + entry.getValue().commonName + " | " + entry.getValue().cid+ " [" + Math.floor(entry.getValue().lastLoc.getX()) + " X - " + Math.floor(entry.getValue().lastLoc.getY())+ " Y - " + Math.floor(entry.getValue().lastLoc.getZ())+ " Z]");
						...
						if (args.length > 1) {
							...
							if (plugin.genera.configs.containsKey(genus)) {
								...
								if (safeNewBlock!=null) {
									...
									if (ZombieMod.debugMode) { ZombieMod.logger.info("[" + ZombieMod.myName + "] " + zomb.commonName +" spawned via command"); }
