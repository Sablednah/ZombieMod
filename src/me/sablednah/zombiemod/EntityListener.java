...
public class EntityListener implements Listener {
	...
	public void onCreatureSpawn(CreatureSpawnEvent event) {
		...
		if (temp instanceof Monster) {
			if (Utils.isNotCalled(l, ZombieMod.factionsWildName)) { // safe location (non wilderness)
	...
	public void onEntityDamage(EntityDamageEvent event) {
		...
		if (zomb != null && zomb.health > -1) {
			...
			if (zt.genus.health < 1) { // kill entity
				...
				if (ZombieMod.hasHeroes) {
					Heroes heroes = (Heroes) plugin.getServer().getPluginManager().getPlugin("Heroes");
					com.herocraftonline.heroes.characters.Monster monsta = heroes.getCharacterManager().getMonster(tEnt);
				...
				if (zt.genus.effects != null) {
					if (zt.genus.effects.contains(Effect.ENDER_SIGNAL)) {
						entity.getWorld().playSound(entity.getLocation(), Sound.ENDERMAN_HIT, 1, 1);
					} else if (zt.genus.effects.contains(Effect.GHAST_SHRIEK)) {
						if (Math.random() > 0.9) {
					...
					} else if (zt.genus.effects.contains(Effect.BOW_FIRE)) {
						entity.getWorld().playSound(entity.getLocation(), Sound.SKELETON_HURT, 1, 1);
					}
				}
				if (zt.genus.species.contains("PlayerZombie") || (zt.genus.abilities != null && zt.genus.abilities.contains("GHOST") )) {
	 ...
	 * // chunk loader
	 * 
	 * @EventHandler public void onChunkLoad(ChunkLoadEvent event) { for (Entity ent: event.getChunk().getEntities()) {
	 * if (ent instanceof Zombie) { zombies.put(ent.getUniqueId(), new PutredineImmortui(SpawnReason.NATURAL.name()));
