...
public final class DexScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partial) {
        super.extractRenderState(g, mouseX, mouseY, partial); // dimmed world
        hotspots.clear();
        ...
        g.text(font, "§6✦", x + 4, y + 8, 0xFFFFFFFF);
        g.text(font, "§6✦", x + w - 12, y + 8, 0xFFFFFFFF);
        g.text(font, "§6✦", x + 4, y + h - 13, 0xFFFFFFFF);
        g.text(font, "§6✦", x + w - 12, y + h - 13, 0xFFFFFFFF);
        ...
        g.text(font, "§6❖", x + w / 2 - 3, divY - 4, 0xFFFFFFFF);

        // Footer tally, centred in the binding.
        var entries = DexState.entries();
        g.centeredText(font, "§8✦ §7" + DexState.slain() + "§8/§7" + entries.size()
    ...
    private void drawList(GuiGraphicsExtractor g, int mouseX, int mouseY, List<DexPayload.Entry> entries) {
        int listX = bookX() + 6;
        int listTop = paneY() - 1;
        ...
        for (DexPayload.Entry e : entries) {
            ...
            g.text(font, mark + colour + trim(e.name(), LIST_W - 22), listX + 3, ly, 0xFFFFFFFF);
    ...
    private void drawIntro(GuiGraphicsExtractor g, List<DexPayload.Entry> entries) {
        int px = paneX();
        int py = paneY() + 6;
        int pw = paneW() - 4;

        if (entries.isEmpty()) {
            g.text(font, "§8Nothing yet — the server has not sent one.", px, py, 0xFFFFFFFF);
            return;
        }
        g.text(font, "§6§lA Field Guide to the Dead", px, py, 0xFFFFFFFF);
        py += 14;
        for (FormattedCharSequence line : font.split(FormattedText.of(
            ...
            g.text(font, line, px, py, 0xFFFFFFFF);
            py += 10;
        }
        py += 6;
        g.text(font, "§a✔ §7slain   §e? §7met   §8✘ not yet found", px, py, 0xFFFFFFFF);
        py += 14;
        for (FormattedCharSequence line : font.split(FormattedText.of(
    ...
    private void drawEntry(GuiGraphicsExtractor g, int mouseX, int mouseY, DexPayload.Entry current) {
        int px = paneX();
        int py = paneY();
        ...
        if (holder == null) {
            g.text(font, "§8This entry is missing from the loaded datapacks.", px, py + 6, 0xFFFFFFFF);
            return;
        }
        ...
        if (current.kills() > 0) {
            ...
            g.text(font, tally, px + pw - font.width(tally) - 6, cy, 0xFFFFFFFF);
        }
        cy += 13;
        ...
        if (genus.description().isPresent()) {
            for (FormattedCharSequence line : font.split(FormattedText.of(
                ...
                g.text(font, line, px, cy, 0xFFFFFFFF);
                cy += 10;
            }
            cy += 4;
        }
        ...
        for (DexEntry.Row row : DexEntry.stats(genus)) {
            g.text(font, "§8" + row.label() + ": §f" + row.detail(), px, cy, 0xFFFFFFFF);
            cy += 10;
        }
        ...
        if (!abilities.isEmpty()) {
            ...
            g.text(font, "§8 Abilities ", px + 8, cy, 0xFFFFFFFF);
            cy += 11;
            for (int i = 0; i < abilities.size(); i++) {
                ...
                g.text(font, colour + (open ? "▼ " : "▶ ") + a.label(), px, cy, 0xFFFFFFFF);
                final int index = i;
                ...
                if (open && !a.detail().isEmpty()) {
                    for (FormattedCharSequence line : font.split(
                        ...
                        g.text(font, line, px + 10, cy, 0xFFFFFFFF);
                        cy += 10;
                    }
                    ...
                    if (sound != null) {
                        boolean pHover = mouseX >= px + 10 && mouseX < px + 60
                                && mouseY >= cy - 1 && mouseY < cy + 10
                                && mouseY >= py && mouseY < py + ph;
                        g.text(font, (pHover ? "§e" : "§6") + "▶ play", px + 10, cy, 0xFFFFFFFF);
                        var play = sound;
        ...
        if (genus.loot().isPresent()) {
            cy += 4;
            g.fill(px, cy + 3, px + pw - 8, cy + 4, INK_DIVIDER);
            g.text(font, "§8 Leaves behind ", px + 8, cy, 0xFFFFFFFF);
            cy += 12;
            if (current.drops().isEmpty()) {
                g.text(font, "§8Kill one to learn what it leaves behind.", px, cy, 0xFFFFFFFF);
                cy += 11;
            } else {
                for (String dropId : current.drops()) {
                    ...
                    if (item != null) {
                        ...
                        g.item(stack, px + 2, cy - 3);
                        g.text(font, "§7" + stack.getHoverName().getString(), px + 22, cy, 0xFFFFFFFF);
                    } else {
                        g.text(font, "§7" + dropId, px + 2, cy, 0xFFFFFFFF);
                    }
                    cy += 14;
                }
            }
        ...
        if (doll != null) {
            ...
            InventoryScreen.extractEntityInInventoryFollowsMouse(g, dx0 + 2, centerY - half,
    ...
    private void chip(GuiGraphicsExtractor g, int x, int y, int w, String label, boolean enabled,
        ...
        g.text(font, colour + label, x + (w - font.width(label)) / 2, y + 3, 0xFFFFFFFF);
        if (enabled) {
