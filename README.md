...
> **3.5.1.** 61 genera, every ability from the 1.8 plugin rebuilt, and the great majority of it
> confirmed in play. New in 3.5: **permission nodes**, so a storyteller can be handed
> hordes and spawning without being made an op, and the **Undertow stays in the water**; 3.5.1 stops
> another mod's fake player crashing the server. See
...
**Advancements are a vanilla feature.** They are datapack files, judged on the server and drawn by an
unmodded client — toast, tab, chat line and all — so nobody needs ZombieMod installed to earn or see
them. Screen replacements such as [Better Advancements](https://modrinth.com/mod/better-advancements)
...
The shipped advancements are ordinary files under `data/zombiemod/advancement/`, so a datapack can
override or empty any of them. More usefully, **a pack that adds a genus can add advancements for it
with no code**. ZombieMod registers no trigger of its own — a custom trigger is one more thing to ask
a vanilla client about — and instead grants criteria **by name**. Give any advancement, in any
namespace, a criterion using vanilla's do-nothing trigger and one of these names:

```json
"criteria": {
  "zombiemod:kill/mypack:runner": { "trigger": "minecraft:impossible" }
}
```

| Criterion name | Granted when the player… |
|---|---|
...
| `zombiemod:horde_cleared/<horde>` | clears a horde, to the last straggler |
| `zombiemod:cured/<entity type>` | cures an infection with milk |
...
nothing granted the tab never appears. `/zombiemod status` counts grants and the criterion names in
use, and warns in the log about a `met_count/` that is not followed by a whole number.

The shipped set is generated by `scripts/make-advancements.py`, so that an icon wearing a genus's
face takes the texture from that genus's own file.
...
| `advancements.enabled` | `true` | The ZombieMod advancements tab. Vanilla advancements; no client mod needed. |
